...
fn optimize_and_print_status(model: &mut Model) -> Result<()> {
    r#try!(model.optimize());

    println!("IsMIP = {}", r#try!(model.get(attr::IsMIP)) != 0);
    for v in model.get_vars() {
        let vname = r#try!(v.get(&model, attr::VarName));
        let x = r#try!(v.get(&model, attr::X));
        println!("{} = {}", vname, x);
    }
    println!("Obj = {}\n", r#try!(model.get(attr::ObjVal)));
    Ok(())
}
